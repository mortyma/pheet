/*
 * KDelayedLSMLocalityTaskStorage.h
 *
...
#ifndef KDELAYEDLSMLOCALITYTASKSTORAGE_H_
#define KDELAYEDLSMLOCALITYTASKSTORAGE_H_

#include "KDelayedLSMLocalityTaskStoragePlace.h"
...
class KDelayedLSMLocalityTaskStorage : public Pheet::Scheduler::BaseTaskStorage {
public:
	typedef KDelayedLSMLocalityTaskStorage<Pheet, Strategy> Self;
	...
	typedef KDelayedLSMLocalityTaskStoragePlace<Pheet, Self, typename ParentTaskStorage::Place, Strategy> Place;
	typedef typename Place::GlobalListItem GlobalListItem;

	KDelayedLSMLocalityTaskStorage(ParentTaskStorage* parent)
	:parent(parent) {
	...
	~KDelayedLSMLocalityTaskStorage() {
		delete[] places;
	...
	T steal(BaseItem* boundary, procs_t place_id) {
		...
		// For KDelayedLSM stealing is called in the context of the stealer, therefore need to retrieve place
	...
	static void print_name() {
		std::cout << "KDelayedLSMLocalityTaskStorage";
	}
...
template <class Pheet, class Strategy>
std::atomic<KDelayedLSMLocalityTaskStorage<Pheet, Strategy>*>
KDelayedLSMLocalityTaskStorage<Pheet, Strategy>::singleton(nullptr);

} /* namespace pheet */
#endif /* KDELAYEDLSMLOCALITYTASKSTORAGE_H_ */
