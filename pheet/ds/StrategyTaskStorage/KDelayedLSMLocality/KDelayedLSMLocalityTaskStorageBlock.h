...
public:
	...
	KDelayedLSMLocalityTaskStorageBlock(size_t size, size_t max_level)
	:filled(0), owned_filled(0), size(size), max_level(max_level), level(0), level_boundary(1),
	 next(nullptr), prev(nullptr), k(std::numeric_limits<size_t>::max()),
